...
public class CreateChatListener {
    ...
    public void listen(@Payload CreateChatEvent event, Acknowledgment ack) {
        try {
            ...
            log.error("Business error processing chat creation event: {}. Error: {}", event, e.getMessage());
            ack.acknowledge();
        } catch (Exception e) {
    ...
    public void listenDlt(@Payload CreateChatEvent event, Acknowledgment ack) {
        try {
            ...
            messageService.sendMessageToErrorChat("Ошибка обработки задания на создание чата: " + event);
        } catch (Exception e) {
